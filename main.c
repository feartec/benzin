...
void ReadDataFromMemoryX(void* destination, void* input, size_t size)
{
...
void ReadDataFromMemory(void* destination, void* input, size_t size)
{
	ReadDataFromMemoryX(destination, input, size);
	fileoffset += size;
}

void CreateGlobal_pai1(pai1_header_type2 *pai1_header, pai1_header_type1 pai1_header1, pai1_header_type2 pai1_header2, int pai1_header_type)
{
	if(pai1_header_type == 1) {
		...
		memcpy(pai1_header, &pai1_header2, sizeof(pai1_header_type2));
}

void DisplayRLPAData(u32 flags, RLPA_data data)
{
...
int FourCCsMatch(fourcc cc1, fourcc cc2)
{
...
void DisplayRLPAInformation(int idx, RLPA_header* heads, RLPA_entry** entriesx,
...
void ReadRLPAFromBRLAN(int idx, u8* brlan_file, RLPA_header *head, RLPA_entry** entries,
		  RLPA_entryinfo** entryinfo, RLPA_data*** data)
{
	int rlpahead_location = fileoffset;
	ReadDataFromMemory(&head[idx], brlan_file, sizeof(RLPA_header));
	...
	for(i = 0; i < head[idx].entry_count; i++) {
		...
		ReadDataFromMemory(&entries[idx][i], brlan_file, sizeof(RLPA_entry));
		dbgprintf("read entry.\n");
	}
	for(i = 0; i < head[idx].entry_count; i++) {
		dbgprintf("i %d cnt %d\n", i, head[idx].entry_count);
		fileoffset = be32(entries[idx][i].offset) + rlpahead_location;
		dbgprintf("read entry infos. 0x%08x\n", &entryinfo[idx][i]);
		ReadDataFromMemory(&entryinfo[idx][i], brlan_file, sizeof(RLPA_entryinfo));
		...
		for(z = 0; z < be16(entryinfo[idx][i].coord_count); z++) {
			...
			ReadDataFromMemory(&data[idx][i][z], brlan_file, sizeof(RLPA_data));
			dbgprintf("read data.\n");
		}
	}
}
...
void parse_brlan(char* filename)
{
	...
	brlan_header header;
	ReadDataFromMemoryX(&header, brlan_file, sizeof(brlan_header));
	dbgprintf("brlan_header read to.\n");
	fileoffset = be16(header.pai1_offset);
	pai1_universal universal;
	ReadDataFromMemoryX(&universal, brlan_file, sizeof(pai1_universal));
	dbgprintf("pa1_universal read to.\n");
	
	int pai1_header_type;
	pai1_header_type1 pai1_header1;
	pai1_header_type2 pai1_header2;
	pai1_header_type2 pai1_header;

	if((be32(universal.flags) & (1 << 25)) >= 1) {
		pai1_header_type = 2;
		ReadDataFromMemory(&pai1_header2, brlan_file, sizeof(pai1_header_type2));
	} else {
		pai1_header_type = 1;
		ReadDataFromMemory(&pai1_header1, brlan_file, sizeof(pai1_header_type1));
	}
	...
	ReadDataFromMemory(taglocations, brlan_file, tagcount * sizeof(u32));
	...
	for(i = 0; i < tagcount; i++) {
		fileoffset = be32(taglocations[i]) + be16(header.pai1_offset);
		dbgprintf("fileoffset set.\n");
		ReadDataFromMemory(&(tag_entries[i]), brlan_file, sizeof(brlan_entry));
		...
		u8 magick[4];
		ReadDataFromMemoryX(magick, brlan_file, 4);
		memcpy(CCs[i], magick, 4);
		dbgprintf("read fourcc from %04x %02x%02x%02x%02x.\n", fileoffset, magick[0], magick[1], magick[2], magick[3]);
